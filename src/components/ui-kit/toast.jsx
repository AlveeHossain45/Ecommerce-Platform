...
import { CheckCircle, XCircle, Info, AlertTriangle, X, Sparkles, Zap, Bell, Download, ExternalLink } from 'lucide-react'
...
const Toast = ({ toast, onRemove, position = 'top-right' }) => {
  ...
  return (
    <motion.div
      ...
      className={clsx(
        'flex items-start p-4 border rounded-xl shadow-2xl backdrop-blur-sm max-w-sm w-full relative overflow-hidden transform-gpu',
        colorClass.bg,
        colorClass.border,
        colorClass.text,
        colorClass.glow
    ...
    >
      ...
      {toast.duration && toast.duration !== Infinity && (
        <motion.div
          className={clsx(
            'absolute top-0 left-0 h-1 bg-gradient-to-r rounded-t-xl',
            colorClass.gradient
          )}
          initial={{ width: '100%' }}
          animate={{ width: '0%' }}
      ...
      {/* Icon with Animation */}
      <motion.div
        className={clsx('p-2 rounded-lg mr-3', colorClass.bg)}
...
export const ToastContainer = ({ toasts, onRemove, position = 'top-right' }) => {
  ...
  return (
    <div className={clsx('fixed z-50 space-y-3', positionClasses[position], 'flex flex-col')}>
